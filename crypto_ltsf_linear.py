...
def load_crypto_data(file_path, target_col='Close'):
...
def train_model(model, train_loader, valid_loader, configs, epochs=10):
    """Train the model"""
...
def run_experiment(data_path, model_name, configs):
    ...
    # Create datasets
    train_dataset = CryptoDataset(train_data_scaled, configs.seq_len, configs.pred_len, scaler)
    valid_dataset = CryptoDataset(valid_data_scaled, configs.seq_len, configs.pred_len, scaler)
    test_dataset = CryptoDataset(test_data_scaled, configs.seq_len, configs.pred_len, scaler)
...
def main():
    ...
    results_df.to_csv('/Users/hieuduc/Downloads/rmit/ltsf_linear_results.csv', index=False)
    ...
    # Save summary
    summary.to_csv('/Users/hieuduc/Downloads/rmit/ltsf_linear_summary.csv')
    
    print(f"\nDetailed results saved to: ltsf_linear_results.csv")
    print(f"Summary results saved to: ltsf_linear_summary.csv")
