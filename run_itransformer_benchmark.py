...
def train_itransformer_model(model, train_loader, valid_loader, configs, epochs=10):
...
def evaluate_itransformer_model(model, test_loader, scaler, configs):
    """Evaluate the iTransformer model on test set"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    model.eval()
...
def run_itransformer_experiment(data_path, configs):
    ...
    trained_model = train_itransformer_model(model, train_loader, valid_loader, configs)
    
    # Evaluate model
    print("Evaluating iTransformer...")
    mae, mse, rmse, trend_match = evaluate_itransformer_model(trained_model, test_loader, scaler, configs)
...
def main():
    ...
    for i, csv_file in enumerate(csv_files):
        ...
        try:
            ...
            print(f"Error running iTransformer on {csv_file}: {str(e)}")
            import traceback
            traceback.print_exc()
            continue
    ...
    if all_results:
        ...
        results_df.to_csv('/Users/hieuduc/Downloads/rmit/itransformer_results.csv', index=False)
        
        # Print summary
        print(f"\n{'='*100}")
        print("ITRANSFORMER SUMMARY RESULTS")
        print(f"{'='*100}")
        ...
        summary_df.to_csv('/Users/hieuduc/Downloads/rmit/itransformer_summary.csv', index=False)
        
        # Print detailed results
        print(f"\nDetailed results by dataset:")
        print(results_df.to_string(index=False))
        
        print(f"\nDetailed results saved to: itransformer_results.csv")
        print(f"Summary results saved to: itransformer_summary.csv")
    else:
